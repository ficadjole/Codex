...
export class OrderController {
  ...
  private async getMyOrders(req: Request, res: Response): Promise<void> {
    try {
  ...
  private async getFullOrderDetails(
    ...
    try {
      ...
      if (user.userRole !== UserRole.ADMIN && result.order.userId !== user.id) {
        res.status(403).json({
          success: false,
          message: "You are not allowed to view this order.",
        });
  ...
  private async changeStatus(req: Request, res: Response): Promise<void> {
    try {
      const orderId = parseInt(req.params.orderId);
